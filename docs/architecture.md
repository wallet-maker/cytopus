...
- `KnowledgeBase` (in `cytopus/knowledge_base/kb_queries.py`): central class wrapping a NetworkX DiGraph and exposing query and plotting methods.
- `tl` tooling (in `cytopus/tl/`): helper modules to construct KBs, manipulate hierarchies, and label/export gene sets.
- `data/` files: packaged pickled NetworkX graphs and example `AnnData` dataset used as defaults.
...
- `cytopus/knowledge_base/kb_queries.py` — loading KB data, filtering utilities, constructing process/identity dictionaries, plotting (matplotlib/pygraphviz/pyvis wrappers), and main `KnowledgeBase` API.
- `cytopus/tl/create.py` — graph construction from lists/metadata; returns `KnowledgeBase` instances.
- `cytopus/tl/hierarchy.py` — nested hierarchy <-> graph conversion utilities and the `Hierarchy` class for cell attachments and queries.
- `cytopus/tl/label.py` — overlap computations and labeling utilities focused on mapping marker gene lists or factor loadings to KB gene sets; also CSV/GMT export helpers.
